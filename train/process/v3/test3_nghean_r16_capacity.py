...
EXP_DIR = TRAIN_DIR / "output" / "nghean_test3_r16_capacity"
DATA_DIR = TRAIN_DIR / "output" / "nghean_test2_accent_scale"
OUT = EXP_DIR
CACHE_DIR = OUT / "cache"
...
def read_pairs(name):
    with (DATA_DIR / name).open("r", encoding="utf-8-sig", newline="") as f: rows = list(csv.DictReader(f))
    for r in rows:
...
def inject(engine):
    ...
    inject_ffn_lora(engine.model.acoustic_decoder.layers[0], 16, 32., 0.)
...
def save_adapter(engine, path, step, val, mode):
    ...
    (path/"adapter_config.json").write_text(json.dumps({"rank":16,"alpha":32,"dropout":0.,"best_step":step,"validation_ce":val,"mode":mode},indent=2),encoding="utf-8")
